...
const Partners = () => {
  ...
  return (
    <div className='w-full py-16 min-h-content flex flex-col relative'>
        ...
        <div className='w-full max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6'>
            {partner.map((partner) => (
                <div key={partner.name} className='flex flex-col items-center justify-center'>
                    <Image src={partner.image} alt={partner.name} width={50} height={50} className='w-40 h-40 object-contain' />
                </div>
            ))}
        </div>
    </div>
